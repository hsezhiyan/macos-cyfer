...
NSString* globalCurrentWebsite = nil;
...
- (NSMutableDictionary*) addCurrentApplication:(NSMutableDictionary *)timerDict {
    ...
    if ([currentApp length] != 0) {
        if ([browserTracker isBrowser:currentApp] && [globalCurrentWebsite length] != 0) {
            CFTimeInterval elapsedTime = currentTime - webStartTime;
            [timerDict setObject:globalCurrentWebsite forKey:@"CurrentWebsiteName"];
            [timerDict setObject:[NSNumber numberWithFloat:elapsedTime] forKey:@"CurrentWebsiteTime"];

        } else {
            CFTimeInterval elapsedTime = currentTime - startTime;
            [timerDict setObject:currentApp forKey:@"CurrentAppName"];
            [timerDict setObject:[NSNumber numberWithFloat:elapsedTime] forKey:@"CurrentAppTime"];
        }
    }
    return timerDict;
}
...
- (void) appPolling:(NSNotification *)notification {
    ...
    NSMutableDictionary* updatedDict = [self getUpdatedEventsOnly:oldDict :newDict];
